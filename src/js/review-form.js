...
/*
  Creating html content that will return li list of reviews .
 */
...
/**
 Validating the fields of form so that if any field is not filled then it control will focus on that..
 */
...
function handleSubmit(e) {
  ...
  const POST = {
    ...
    console.log("vvvvvvvvvvvvvvviiiiiiiiiiii__Onnnnnnnnnnnnnn",JSON.stringify(review));
  ...
  return fetch(url, POST).then(response => {
    if (!response.ok) { //clearForm();
	                     /* DBHelper.createIDBReview(review)
	                      ...
	                      window.location.href =
                             `/restaurant.html?id=${review.restaurant_id}&isOffline=true`;  */return Promise.reject("We couldn't post review to server.")
							 };
    ...
    // clear form
    clearForm();
  }).catch(err =>{   console.log("hgggggggggggggggggggggggggggggggggggj")
                     clearForm();
	                      DBHelper.createIDBReview(review)
                          .then(review_key => {
                            //Get review_key 
	                   ...
	                   //  window.location.href =`/restaurant.html?id=${review.restaurant_id}&isOffline=true`; 
  
                     DBHelper.showOffline();  
  });
  
  dbPromise.getAllIdx('reviews', 'restaurant_id', restaurant_id)  
        .then(reviews => {                                            
          // console.log(reviews);                                    
...
export default function reviewForm(restaurantId) {
  ...
  textarea.setAttribute('placeholder', 'Enter your reviews  here');
